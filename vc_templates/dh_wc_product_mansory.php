				...
				<ul data-filter-key="filter">
					...
					<?php foreach ($category_arr as $cat):?>
						<?php if($cat):?>
							...
							<?php if($category): ?>
							<li>
								<a <?php if(!empty($hide_all_filter) && !$filter_flag):$filter_flag=true?>  class="selected"  data-masonry-toogle="selected" <?php endif;?> href="#" data-filter-value= ".<?php echo esc_attr($category->slug) ?>"><?php echo esc_html($category->name); ?></a>
							</li>
							<?php endif;?>
						<?php endif;?>
					<?php endforeach;?>
